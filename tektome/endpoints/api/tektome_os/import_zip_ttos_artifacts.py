...
from ...models.import_zip_artifacts_request import ImportZipArtifactsRequest
from ...models.import_zip_ttos_artifacts_response import ImportZipTtosArtifactsResponse
...
def _get_kwargs(
    chatroom_id: UUID,
    *,
    body: ImportZipArtifactsRequest,
) -> dict[str, Any]:
    headers: dict[str, Any] = {}

    _kwargs: dict[str, Any] = {
        "method": "post",
        "url": "/api/core/agents/os/chatrooms/{chatroom_id}/artifacts/import-zip/".format(
...
) -> GenericHttpError | ImportZipTtosArtifactsResponse | None:
    if response.status_code == 202:
        response_202 = ImportZipTtosArtifactsResponse.from_dict(response.json())
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[GenericHttpError | ImportZipTtosArtifactsResponse]:
    return Response(
...
def sync_detailed(
    ...
    *,
    ...
    """Import artifacts from a zip archive URL

     Queue a background job to download a zip archive and create artifacts
    from its text files. Binary files are automatically skipped; only UTF-8
    decodable text files are imported. If the zip contains a single common
    root folder (e.g. GitHub archives), it is automatically stripped.
...
def sync(
    ...
    *,
    ...
    Returns:
        GenericHttpError | ImportZipTtosArtifactsResponse
    """
