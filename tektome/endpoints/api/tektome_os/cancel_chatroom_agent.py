...
def _get_kwargs(
    ...
    _kwargs: dict[str, Any] = {
        "method": "post",
        "url": "/api/core/agents/os/chatrooms/{chatroom_id}/cancel/".format(
...
def sync_detailed(
    ...
    *,
    ...
    """Cancel chatroom agent

     Request cancellation of the running agent in a chatroom. The agent stops at its next checkpoint
    (between graph iterations or during streaming) and the chatroom returns to idle. Safe to call
    multiple times; only valid while the agent is pending or processing.
