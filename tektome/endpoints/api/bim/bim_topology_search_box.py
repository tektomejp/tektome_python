...
def _get_kwargs(
    ...
    *,
    ...
    _kwargs: dict[str, Any] = {
        "method": "post",
        "url": "/api/core/resource-groups/bim/bim-tools/topology-search/{bim_project_id}/box/".format(
...
def sync_detailed(
    ...
    *,
    ...
    in the BIM project. Search using boundaries parallel to the cartesian coordinate system axes. This
    is an asynchronous operation. To retrieve the results, use the get_celery_task
    (/api/core/tasks/{task_id}/) endpoint with the task/process ID returned in this response.

    Args:
        bim_project_id (UUID):
        body (BimTopologySearchBoxPostIn): Schema for BIM topology search with box request.
