...
from ...models.resource_delete_in import ResourceDeleteIn
from ...models.resource_delete_out import ResourceDeleteOut
...
def _get_kwargs(
    project_id: UUID,
    *,
    body: ResourceDeleteIn,
) -> dict[str, Any]:
    headers: dict[str, Any] = {}

    _kwargs: dict[str, Any] = {
        "method": "post",
        "url": "/api/core/resource-paths/projects/{project_id}/delete/".format(
...
) -> GenericHttpError | ResourceDeleteOut | None:
    if response.status_code == 200:
        response_200 = ResourceDeleteOut.from_dict(response.json())
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[GenericHttpError | ResourceDeleteOut]:
    return Response(
...
def sync_detailed(
    ...
    *,
    ...
    """Delete resource by path
     ...
     Delete a resource or folder at the given path. For folder paths (.keep files), recursively deletes
    all resources under the folder.
...
def sync(
    ...
    *,
    ...
    Returns:
        GenericHttpError | ResourceDeleteOut
    """
