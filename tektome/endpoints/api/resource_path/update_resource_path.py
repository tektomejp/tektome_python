...
from ...models.resource_patch_path_in import ResourcePatchPathIn
from ...models.resource_patch_path_out import ResourcePatchPathOut
...
def _get_kwargs(
    rvc_id: UUID,
    *,
    body: ResourcePatchPathIn,
) -> dict[str, Any]:
    headers: dict[str, Any] = {}

    _kwargs: dict[str, Any] = {
        "method": "patch",
        "url": "/api/core/resource-paths/{rvc_id}/".format(
            rvc_id=quote(str(rvc_id), safe=""),
        ),
    }
...
) -> GenericHttpError | ResourcePatchPathOut | None:
    if response.status_code == 200:
        response_200 = ResourcePatchPathOut.from_dict(response.json())
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[GenericHttpError | ResourcePatchPathOut]:
    return Response(
...
def sync_detailed(
    ...
    *,
    ...
    """Update resource path
     ...
     Update the path of a specific resource version control entry. Validates the new path and checks for
    conflicts.
...
def sync(
    ...
    *,
    ...
    Returns:
        GenericHttpError | ResourcePatchPathOut
    """
