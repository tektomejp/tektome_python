...
from ...models.attribute_context_response import AttributeContextResponse
...
def _get_kwargs(
    ...
    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": "/api/core/attributes/{attribute_id}/context/".format(
...
) -> AttributeContextResponse | GenericHttpError | None:
    if response.status_code == 200:
        response_200 = AttributeContextResponse.from_dict(response.json())
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[AttributeContextResponse | GenericHttpError]:
    return Response(
...
def sync_detailed(
    ...
    *,
    ...
    """Get context for a resource attribute

     Resolve the dataspace, project, and resource associated with an attribute. Only attributes owned by
    an entity in the authenticated user's organization are accessible. Returns the dataspace ID, project
    ID, and resource ID, only applicable to resource attributes.
...
def sync(
    ...
    *,
    ...
    Returns:
        AttributeContextResponse | GenericHttpError
    """
