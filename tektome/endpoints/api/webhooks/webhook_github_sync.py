...
from ...models.git_hub_webhook_response import GitHubWebhookResponse
...
def _get_kwargs() -> dict[str, Any]:
    ...
    _kwargs: dict[str, Any] = {
        "method": "post",
        "url": "/api/core/webhooks/github/system-process-template-sync/",
    }
...
) -> GenericHttpError | GitHubWebhookResponse | None:
    if response.status_code == 200:
        response_200 = GitHubWebhookResponse.from_dict(response.json())
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[GenericHttpError | GitHubWebhookResponse]:
    return Response(
...
def sync_detailed(
    *,
    ...
    """GitHub webhook for template sync

     Receives GitHub push events and triggers system template sync. Verifies the webhook signature using
    X-Hub-Signature-256 header. Only processes push events to the configured branch.
...
def sync(
    *,
    ...
    Returns:
        GenericHttpError | GitHubWebhookResponse
    """
