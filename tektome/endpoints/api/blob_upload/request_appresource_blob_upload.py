...
from ...models.blob_upload_request_in import BlobUploadRequestIn
from ...models.blob_upload_response_out import BlobUploadResponseOut
...
def _get_kwargs(
    project_id: UUID,
    *,
    body: BlobUploadRequestIn,
) -> dict[str, Any]:
    headers: dict[str, Any] = {}

    _kwargs: dict[str, Any] = {
        "method": "post",
        "url": "/api/core/resources/{project_id}/upload/blob/request/".format(
...
) -> BlobUploadResponseOut | ErrorOut | None:
    if response.status_code == 200:
        response_200 = BlobUploadResponseOut.from_dict(response.json())
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[BlobUploadResponseOut | ErrorOut]:
    return Response(
...
def sync_detailed(
    ...
    *,
    ...
    """Request a blob upload URL
     ...
     Request a pre-signed upload URL for uploading large files directly to storage. After uploading the
    file to the returned URL, call the upload complete endpoint to finalize the resource creation.

    Args:
        project_id (UUID):
        body (BlobUploadRequestIn): Request to initiate a blob upload.
...
def sync(
    ...
    *,
    ...
    Returns:
        BlobUploadResponseOut | ErrorOut
    """
