...
T = TypeVar("T", bound="DateSearchCondition")


@_attrs_define
class DateSearchCondition:
    """Data Model for date conditions. Expect attribute_type to be "date"

    Attributes:
        attribute_name (str): The name of the attribute to filter on
        attribute_type (Literal['date']): The type of the attribute being filtered (date)
        ...
        value (datetime.date): The value to filter by
    """

    attribute_name: str
    attribute_type: Literal["date"]
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        attribute_type = cast(Literal["date"], d.pop("attribute_type"))
        if attribute_type != "date":
            raise ValueError(f"attribute_type must match const 'date', got '{attribute_type}'")

        action = DateTimeConditionAction(d.pop("action"))

        value = isoparse(d.pop("value")).date()

        date_search_condition = cls(
            attribute_name=attribute_name,
            attribute_type=attribute_type,
            action=action,
            value=value,
        ...
        date_search_condition.additional_properties = d
        return date_search_condition
