...
Use `post_general_resource_attribute` to create an attribute instance on a resource. This returns `AttributeGetOut` with the attribute's `id`, which is required for citation creation.
...
```python
from tektome.endpoints.api.dataspace import post_general_resource_attribute
from tektome.endpoints.models.resource_attribute_post_in import ResourceAttributePostIn
from tektome.endpoints.models.attribute_type import AttributeType

attr = post_general_resource_attribute.sync(
    dataspace_id=dataspace_uuid,
    client=client,
...
For a project-level attribute, use `post_general_project_attribute` instead (same signature, different URL).
...
Table attributes require a config to be declared first (Step 0) with the column schema in `attribute_metadata`. Once the config exists, write data directly via `upsert_table_attributes`.

> **Do NOT call `post_general_resource_attribute` for table attributes — before or after upsert.** These endpoints operate on different underlying attribute representations. Calling it before the upsert creates a conflicting empty attribute instance. Calling it after (even with `value=[]`) overwrites the populated table with an empty `[]`, destroying your rows. To get the attribute ID for citations, read it from `get_dataspace_project_resource` instead — see "Getting the `attribute_id`" below.

### Write the table data

`upsert_table_attributes` replaces the **entire table** in one call. Send a flat list of `TableCellUpdate` objects — one per cell — using `row_index` and `column` to address each cell. This implicitly defines the table's rows and columns.

```python
from tektome.endpoints.api.attribute import upsert_table_attributes
from tektome.endpoints.models.table_attribute_body_patch_in import TableAttributeBodyPatchIn
from tektome.endpoints.models.table_cell_update import TableCellUpdate
from tektome.endpoints.models.upsert_table_attributes_dataspace_attribute_object_types import (
...
if current_version is not None:
    body = TableAttributeBodyPatchIn(name="bim_objects", cells=cells, version=current_version)
else:
    body = TableAttributeBodyPatchIn(name="bim_objects", cells=cells)  # UNSET — omits version field

upsert_resp = upsert_table_attributes.sync_detailed(
    object_type=UpsertTableAttributesDataspaceAttributeObjectTypes.RESOURCE,
    object_id=resource_uuid,
    client=client,
    body=body,
)
if upsert_resp.status_code.value != 204:
    raise RuntimeError(f"upsert_table_attributes failed: {upsert_resp.content.decode()}")
```

### Version handling for `upsert_table_attributes`
...
if current_version is not None:
    body = TableAttributeBodyPatchIn(name=attr_name, cells=cells, version=current_version)
else:
    body = TableAttributeBodyPatchIn(name=attr_name, cells=cells)  # UNSET — omitted
```

### Getting the `attribute_id` for a table attribute (needed for citations)

> **Do NOT use `post_general_resource_attribute(value=[])` to retrieve the attribute ID.** Even when called *after* `upsert_table_attributes`, this call overwrites the populated table with an empty `[]` instance in the database. The table appears empty despite a successful upsert.
...
- **Full replace**: every call to `upsert_table_attributes` replaces the entire table. To add a row to an existing table, read the current data first, append to it, and send the complete set.
...
- **`insert_table_attribute_row`**: inserts a blank row at a given `row_index`, shifting existing rows down. Use this when you need to splice a row into the middle of an existing table rather than replacing everything.

### Config idempotency

`create_dataspace_resource_attribute_config` returns **400** (not 409) when the config already exists. When handling re-runs, treat both 400 and 409 as "already exists". When the config already exists and you cannot read the `attribute_name` from the response, derive it: the server-generated `attribute_name` is the label lowercased with spaces and hyphens replaced by underscores (e.g. `"Non-AKT Foundations Table"` → `"non_akt_foundations_table"`).
...
elif config_resp.status_code.value in (400, 409):
...
from tektome.endpoints.models.bim_citation_post_in import BIMCitationPostIn
from tektome.endpoints.models.bim_element_in import BIMElementIn
from tektome.endpoints.models.create_attribute_bim_citation_dataspace_entity_type import (
    CreateAttributeBimCitationDataspaceEntityType,
)
    ...
    body=BIMCitationPostIn(
        ...
        bim_elements=[                                   # required — at least one element
            BIMElementIn(
                ...
                bim_object_id="element-123",             # optional: omit to cite the whole project
            ),
            BIMElementIn(
                bim_project_id=bim_project_uuid,
                bim_object_id="element-456",
            ),
        ],
...
from tektome.endpoints.models.bim_citation_patch_in_patch import BIMCitationPatchInPatch
...
bim_elements = [
    BIMElementIn(
        bim_project_id=bim_project_uuid,
        bim_object_id=eid,  # 32-char Speckle hash (the `id` from KV search) — NOT UUID, NOT Revit Tag
    )
...
if resp.status_code.value == 201:
    ...
    update_attribute_bim_citation.sync_detailed(
        ...
        client=client,
        body=BIMCitationPatchInPatch(bim_elements=bim_elements),
    )
```
...
from tektome.endpoints.models.pdf_citation_post_in import PDFCitationPostIn
...
create_attribute_pdf_citation.sync(
    ...
    body=PDFCitationPostIn(
        ...
        polygons=[...],                  # optional: list[PDFPolygonSchemaIn] page regions
...
from tektome.endpoints.models.image_citation_post_in import ImageCitationPostIn
...
from tektome.endpoints.models.attribute_citation_post_in import AttributeCitationPostIn
...
| Use `post_general_resource_attribute` to get `attribute_id` | Returns `AttributeGetOut` with `id` — needed for citations |
...
| BIM `bim_elements` is required | Must include at least one `BIMElementIn`; `bim_object_id` is optional (omit to cite the whole project) |
| `bim_object_id` is the 32-char Speckle hash | Not the UUID, not the Revit Tag — use the `id` returned by the KV search endpoint |
...
| Get table `attribute_id` from resource metadata | After `upsert_table_attributes`, read `get_dataspace_project_resource` → `core_attributes_metadata.<name>.id` — never use `post_general_resource_attribute` for this |
...
| Mistake | Symptom | Fix |
|---|---|---|
| Calling `post_general_resource_attribute(value=[])` for a table at all — before or after upsert | Table attribute value reads back as `[]`; citations reference empty attribute | Never call `post_general_resource_attribute` for table attributes. Get the attribute ID from `get_dataspace_project_resource` → `core_attributes_metadata.<name>.id` |
| Passing `version=None` to `upsert_table_attributes` | Server returns **409 Table has been modified** | Leave `version` as `UNSET` (default) for a new table; pass the integer version for updates |
...
| Using `attribute_label` instead of `attribute_name` as the attribute `name` | `upsert_table_attributes` or `post_general_resource_attribute` fails or targets wrong attribute | The config returns both; use `attribute_name` (the server-generated key) not `attribute_label` (the human label) |
