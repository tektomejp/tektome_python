...
class TestAttributeDefinitionsCreation:
    ...
    def test_create_attribute_definitions_with_valid_data(self, sample_uuid_list):
        ...
        attr_defs = AttributeDefinitions(ids=sample_uuid_list, kind="attribute_definition[]")
        assert attr_defs.ids == sample_uuid_list
        assert attr_defs.kind == "attribute_definition[]"
        assert len(attr_defs.ids) == 3
    ...
    def test_create_attribute_definitions_with_uuid_strings(self):
        ...
        attr_defs = AttributeDefinitions(ids=uuid_strings, kind="attribute_definition[]")
        assert len(attr_defs.ids) == 3
        for i, uuid_obj in enumerate(attr_defs.ids):
    ...
    def test_create_attribute_definitions_with_empty_list(self):
        ...
        attr_defs = AttributeDefinitions(ids=[], kind="attribute_definition[]")
        assert attr_defs.ids == []
        assert len(attr_defs.ids) == 0
    ...
    def test_create_attribute_definitions_with_single_uuid(self, sample_uuid):
        ...
        attr_defs = AttributeDefinitions(ids=[sample_uuid], kind="attribute_definition[]")
        assert len(attr_defs.ids) == 1
        assert attr_defs.ids[0] == sample_uuid
...
class TestAttributeDefinitionsValidation:
    ...
    def test_kind_must_be_attribute_definition_array(self, sample_uuid_list):
        ...
        with pytest.raises(ValidationError) as exc_info:
            AttributeDefinitions(ids=sample_uuid_list, kind="resource[]")
    ...
    def test_kind_cannot_be_project_array(self, sample_uuid_list):
        ...
        with pytest.raises(ValidationError) as exc_info:
            AttributeDefinitions(ids=sample_uuid_list, kind="project[]")
        assert "kind must be 'attribute_definition[]'" in str(exc_info.value)

    def test_kind_cannot_be_singular(self, sample_uuid_list):
        """Test that kind cannot be singular form."""
        with pytest.raises(ValidationError) as exc_info:
            AttributeDefinitions(ids=sample_uuid_list, kind="attribute_definition")
        assert "kind must be 'attribute_definition[]'" in str(exc_info.value)
...
class TestAttributeDefinitionsEquality:
    ...
    def test_attribute_definitions_with_same_values_are_equal(self, sample_uuid_list):
        ...
        attr_defs1 = AttributeDefinitions(ids=sample_uuid_list, kind="attribute_definition[]")
        attr_defs2 = AttributeDefinitions(ids=sample_uuid_list, kind="attribute_definition[]")
    ...
    def test_attribute_definitions_with_different_ids_are_not_equal(self):
        ...
        attr_defs1 = AttributeDefinitions(ids=[uuid.uuid4()], kind="attribute_definition[]")
        attr_defs2 = AttributeDefinitions(ids=[uuid.uuid4()], kind="attribute_definition[]")
        assert attr_defs1 != attr_defs2
    ...
    def test_attribute_definitions_with_different_order_are_not_equal(self):
        ...
        attr_defs1 = AttributeDefinitions(ids=[uuid1, uuid2], kind="attribute_definition[]")
        attr_defs2 = AttributeDefinitions(ids=[uuid2, uuid1], kind="attribute_definition[]")
        assert attr_defs1 != attr_defs2
...
class TestAttributeDefinitionsEdgeCases:
    ...
    def test_large_list_of_uuids(self):
        ...
        attr_defs = AttributeDefinitions(ids=large_list, kind="attribute_definition[]")
        assert len(attr_defs.ids) == 1000
    ...
    def test_duplicate_uuids_allowed(self, sample_uuid):
        ...
        attr_defs = AttributeDefinitions(
            ids=[sample_uuid, sample_uuid],
            kind="attribute_definition[]"
        )
        assert len(attr_defs.ids) == 2
        assert attr_defs.ids[0] == attr_defs.ids[1]

    def test_list_type_preservation(self, sample_uuid_list):
        """Test that list type is preserved."""
        attr_defs = AttributeDefinitions(ids=sample_uuid_list, kind="attribute_definition[]")
        assert isinstance(attr_defs.ids, list)
        for uid in attr_defs.ids:
